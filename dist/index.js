...
if (skip_on && labels.indexOf(skip_on) !== -1)
  return;
var body = github.context.payload && github.context.payload.pull_request && github.context.payload.pull_request.body;
if (!body || !body.length)
  return;
var IGNORE_FROM = "<!-- ignore-task-list-start -->";
var IGNORE_UNTIL = "<!-- ignore-task-list-end -->";
var CHECKS_REGEXP = /^[-*]\s?\[[x\s]?\]\s/i;
var COMPLETED_REGEXP = /^[-*]\s?\[[x]?\]\s/i;
var lines = body.split("\n").map((str) => str.trim());
var from = lines.findIndex((str) => str === IGNORE_FROM);
var until = lines.findIndex((str) => str === IGNORE_UNTIL);
if (from !== -1 && until !== -1) {
  lines = [...lines.slice(0, from), ...lines.slice(until + 1)];
}
if (!lines.length)
  return;
var checks = lines.filter((str) => CHECKS_REGEXP.test(str));
var completed = checks.filter((str) => COMPLETED_REGEXP.test(str));
var remaining = checks.length - completed.length;
if (remaining > 0) {
